...
def main():
    ...
    if all_success:
        print("1. Update amplify/auth/resource.ts - add to the groups array:")
        print()
        print(f'    // {building_name}')
        print(f'    "tenant_{tenant_id}_viewer",')
        print(f'    "tenant_{tenant_id}_reviewer",')
        print()
        print("2. Update amplify/storage/resource.ts - add storage access:")
        print()
        print(f'    // {building_name}')
        print(f'    "{tenant_id}/*": [')
        print(f'      allow.groups(["tenant_{tenant_id}_viewer", "tenant_{tenant_id}_reviewer"]).to(["read"]),')
        print(f'      allow.groups(["admin"]).to(["read", "write", "delete"]),')
        print(f'    ],')
        print()
        print("3. Commit and push to deploy (or run npx ampx pipeline-deploy)")
        print()
        print("4. Create the tenant in the Admin UI:")
        print(f"   - Go to Admin > Tenants > Add Tenant")
        print(f"   - Name: {building_name}")
        print(f"   - Group ID: {tenant_id}")
        print()
        print("5. Upload scanned pages for this tenant:")
        print(f"   python upload_pages.py /path/to/pdfs --tenant {tenant_id}")
        print()
        print("6. To add other users, run this script again or use:")
