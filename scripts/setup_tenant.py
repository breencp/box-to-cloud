...
def main():
    ...
    if all_success:
        print("1. Update amplify/storage/resource.ts to add storage access:")
        print()
        ...
        print(f'    ],')
        print()
        print("2. Deploy the backend changes:")
        print("   npx ampx pipeline-deploy --branch main")
        print()
        print("3. Create the tenant in the Admin UI:")
        print(f"   - Go to Admin > Tenants > Add Tenant")
        print(f"   - Name: {building_name}")
        print(f"   - Group ID: {tenant_id}")
        print()
        print("4. Upload scanned pages for this tenant:")
        print(f"   python upload_pages.py /path/to/pdfs --tenant {tenant_id}")
        print()
        print("5. To add other users, run this script again or use:")
