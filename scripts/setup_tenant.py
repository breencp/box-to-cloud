...
def main():
    ...
    # Get User Pool ID
    print("You can find your User Pool ID in the AWS Console:")
    ...
    if all_success:
        ...
        print()
        print("2. Note the Tenant ID from the database (it will be a UUID)")
        print("   You'll need this for uploading scanned pages.")
        print()
        print("3. To upload scanned pages for this tenant, run:")
        print(f"   TENANT_ID=<tenant-uuid> python upload_pages.py /path/to/pdfs")
        print()
        print("4. To invite users to this tenant:")
        print(f"   - Go to Admin > Invites > Send Invite")
        print(f"   - Select the tenant and role (viewer or reviewer)")
        print()
        print("5. After creating a user in Cognito, add them to the appropriate group:")
